...
Multi-agent work is a chain of handoffs: planner to implementer, implementer to
reviewer, reviewer to learning memory, and finally repo to CI. Inspired by the
handoff-validity framing in Agentic EDA, `task handoff` turns a TASK file into a
bounded, deterministic packet for the next consumer. The packet carries the
task provenance (`taskId`, `status`, `priority`, timestamps, `sourceRef`),
acceptance conditions from Requirements, handoff objects from Output files,
subtask checklist items, recent Log evidence, and local issues such as missing
requirements or missing output anchors.
...
Long TASK logs are useful evidence, but they are poor prompt material. Recent
agent context-management work points to a practical rule: keep the original task
constraints, preserve current progress, and compress or omit stale trace under
an explicit budget. `task digest` is FuguNano's deterministic version of that
rule. It renders bounded context cards with goal/status, Requirements,
unfinished subtasks, Output files, and recent Log evidence, plus `sourceRef`,
`sourceSha256`, and `sourceChars` so omitted history remains auditable.

```bash
fuguectl task digest ~/.claude/tasks/TASK-2026-06-29-028.md
fuguectl task digest ~/.claude/tasks/TASK-2026-06-29-028.md --budget-chars 1600 --tail 4
fuguectl task digest ~/.claude/tasks/TASK-2026-06-29-028.md --json
```

This is not an LLM summary. It selects whole context units in a stable priority
order and records how many units/chars were omitted. Use it when an implementer,
reviewer, or planner needs "what matters now" without replaying the whole trace.

## Review Packets

c-CRAB frames useful code review as feedback that can drive objective repair and
validation, while rubric-guided verification work argues that feedback should be
classified, localized, and short enough to reuse. `review packet` is
FuguNano's deterministic bridge from free-form reviewer output to a
review-fix artifact. It parses `VERDICT: ACCEPTED` / `VERDICT: NEEDS FIX`,
extracts file/line evidence, classifies findings into local rubrics such as
`correctness`, `tests`, `security`, `integration`, and `traceability`, and
emits recommended follow-up checks without calling another model.

```bash
fuguectl review packet /tmp/codex-review.txt
fuguectl review packet /tmp/codex-review.txt --json
cat /tmp/codex-review.txt | fuguectl review packet - --source-ref /tmp/codex-review.txt
```

Packets keep `sourceRef`, `sourceSha256`, and `sourceChars`, and flag review
quality issues such as missing verdicts or findings without file evidence. Use
...
fugue task new|log|done|handoff|digest [handoff: --json --tail n --require-done] [digest: --json --tail n --budget-chars n]
fugue review packet <review-file|-> [--json] [--source-ref ref]
