...
stderr；带 `--task` 的 dispatch 会把耗时 / 输出大小持久化进 TASK log。所以真实
Codex/OpenCode/AGY 运行会留下可观察痕迹，同时不污染模型 stdout 或 durable artifact。
...
| Memory and maintenance | `fuguectl experience add\|list\|recall\|show`、`fuguectl runtime check\|adapt`（provider + 已安装 workflow bundle 漂移）、`fuguectl selftest`                                                         |
