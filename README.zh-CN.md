...
assembly 之前套用同一条来源路由。需要更小 prompt 预算时，在同一条自动注入路径上加 `--experience-limit <n>`，限制注入的经验条数。
...
这个方向借鉴的是 Agent Workflow Memory、AgentHER、MemRL、agent-native memory、budget-tier routing、token economics、store routing 与 workflow provenance 研究里的共同结论：不要回放所有 trace。FuguNano 当前这一步刻意保持朴素：按 workspace、来源、失败模式、检索证据、效用门槛和显式 recall cap 来选择；学习式 budget-tier routing 是后续方向。
...
fugue dispatch <target> --harness fugue-cc|codex|opencode|agy [--timeout-ms n] [--codex-clean] [--harness-arg x] [--out <file>] [--require-output] [--verbose] [--workspace ws [--experience-query q] [--experience-source manual|task] [--experience-limit n]] --template <name>|--prompt-file <file>|--prompt <text>
...
fugue workspace list|show|model|context [context: --experience-source manual|task --experience-limit n]
...
- [Securing LLM-Agent Long-Term Memory Against Poisoning](https://arxiv.org/abs/2606.24322) 和 [From Untrusted Input to Trusted Memory](https://arxiv.org/abs/2606.04329) 提醒了相邻的 origin-bound authority 与 memory write-channel 风险，这是后续 memory security 工作需要处理的边界。
