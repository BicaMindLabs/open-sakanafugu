...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/assertions-296-brightgreen?style=for-the-badge" alt="296 个 fuguectl 断言" />
...
| Memory and maintenance | `fuguectl experience add\|list\|recall\|show`、`fuguectl runtime check\|adapt`（provider + 已安装 workflow skill 漂移）、`fuguectl selftest`                                                          |
...
`runtime check` 也会比较仓库里的 `orchestration/fuguectl/SKILL.md` 和本机已安装的 workflow skill；`runtime adapt --apply` 会同步它，避免本机 agent 指令落后于仓库里的最新工作流。如果 `fugue-cc` 不可用，adapt 仍会同步 skill，但会保留非零退出码，让自动化知道 provider restart/stamp 被跳过了。
