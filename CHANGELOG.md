...
- `fuguectl-dispatch` — wraps `fugue-cc ask` (render → dispatch → log; `--workspace`), with `--verbose` stderr observability and append-safe TASK-log start/terminal status, duration, output-size, failure-kind, and optional artifact metadata for concurrent harness runs.
