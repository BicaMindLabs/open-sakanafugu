...
- **Experience learning (Reflexion-style)** — after a useful TASK finishes, run `"$FO" experience learn <workspace> "<retro-title>" --task "$F"` to distill Requirements + meaningful audit log lines into secret-gated workspace memory. Later `workspace context` and `dispatch --workspace` replay it automatically, so hard-won fixes do not stay buried in old TASK files.
