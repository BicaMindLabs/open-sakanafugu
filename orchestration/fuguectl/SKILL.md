...
- **Live runtime smoke** — after preflight passes, `"$FO" smoke --harness all --codex-clean --timeout-ms 120000 --task "$F" --out-dir /tmp/fugunano-smoke` sends exact single-line token probes to the lite runtimes (Codex, OpenCode, Antigravity; one final newline is tolerated) and records pass/fail, duration, output size, and artifacts in the TASK audit.
