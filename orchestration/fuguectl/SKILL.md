...
- **TASK handoff packets** — before handing a task to a reviewer, another agent, or `experience learn`, run `"$FO" task handoff "$F" [--json] [--tail n] [--require-done]`. It emits a deterministic packet with TASK provenance, acceptance conditions from Requirements, handoff objects from Output files, checklist state, recent Log evidence, and local issues. This is the repo-native cut of handoff validity (Agentic EDA 2606.19795) plus trace-to-harness-flaw evidence discipline (HarnessFix 2606.06324): do not make the next consumer infer the contract from a long mutable log.
- **TASK context digest** — before injecting a long TASK into an implementer/reviewer prompt, run `"$FO" task digest "$F" [--budget-chars n] [--tail n] [--json]`. It is deterministic and model-free: goal/status, requirements, unfinished subtasks, output objects, and recent evidence are selected as whole context units under a hard character budget; omitted trace remains auditable through `sourceRef`, `sourceSha256`, `sourceChars`, and omission counts. This is the practical FuguNano cut of context pruning + summarization papers: keep constraints/current progress, do not replay stale trace blindly.
- **Review packets** — after an independent reviewer writes its verdict to a file, run `"$FO" review packet <review-file|-> [--json] [--source-ref ref]`. It deterministically turns `VERDICT: ACCEPTED` / `VERDICT: NEEDS FIX` text into findings with severity, rubric, file/line evidence, source hash, and follow-up checks. This is the c-CRAB / rubric-guided verification cut: reviews become repair/check artifacts instead of free-form prose.
- **Incident packets** — after a failed review, CI log, timeout, guard block, or missing artifact, run `"$FO" incident packet <failure-log|-> [--json] [--source-ref ref]`. It deterministically labels the failed trace with local failure cause, MAST category, harness layer, line evidence, source hash, and recovery checks. This is the FuguNano cut of HarnessFix + MAST + execution provenance: normalize the failure before repair, memory learning, or Self-Harness mining.
- **Runtime guard packets** — before dispatching a prompt that contains browser/email/GitHub material, destructive shell commands, deployment actions, or secrets-adjacent instructions, run `"$FO" guard prompt <prompt-file|-> [--json] [--source-ref ref]`. It deterministically emits an `ALLOW` / `REVIEW` / `BLOCK` packet with source hash, line evidence, and follow-up checks; `BLOCK` exits nonzero. This is the FuguNano cut of AgentSpec runtime rules, AgentVisor semantic privilege separation, and CaMeL's trusted-control/untrusted-data split.
- **Dispatch action certificates** — when a runtime action needs replay-ready proof, add `"$FO" dispatch <agent> ... --certificate /tmp/action.cert.json [--approval-class operator-reviewed] [--certificate-assumption "..."] [--certificate-externality "..."]`. It writes a PCAA-style JSON sidecar with harness/target, prompt/output hashes, approval class, assumptions, externality facts, outcome, and five checkpoints (pre-action admissibility, action open, assumption capture, approval, outcome closure). Requested certificate write failure makes dispatch fail; never silently lose proof.
...
- **Workspace isolation** (inspired by Zleap-Agent) — `"$FO" workspace context <ws>` assembles layered context per station (System + Workspace + Tools + Memory + History); `"$FO" dispatch <agent> --workspace <ws> ...` injects it as a prefix, so a (weak) model sees only what this station should see, not drowned in the full context. Stations: main/code/sql/chinese/review/web. Experience memory is selected by task/prompt query (`workspace context --task/--query`, `dispatch --experience-query` override) before injection, so agents replay relevant routines instead of merely the latest notes. Add `--experience-source manual|task` to either path when automatic injection should receive only operator notes or only TASK-derived memory, `--experience-source-ref <ref>` when it should replay only memory from one exact write-time origin, `--experience-limit <n>` when fewer candidate records should be recalled, `--experience-budget-chars <n>` when rendered memory must fit a hard prompt budget, `--experience-max-age-days <n>` when older experience should be ignored after a model/dependency/API/workflow change, and `--experience-trust all` only when untrusted records should be explicitly replayed. Superseded memories are hidden from automatic injection by default. Injected memories carry parse-stable JSON `[experience:meta] {"slug":...,"sourceKind":...,"trustKind":...,"created":...}` evidence metadata, plus source-ref/failure/supersession fields when present.
- **Experience learning (Reflexion-style + AgentHER-style failure relabeling)** — after a useful TASK finishes, run `"$FO" experience learn <workspace> "<retro-title>" --task "$F"` to distill Requirements + Output files + meaningful audit log lines into secret-gated workspace memory with `source=task:<TASK.md>` provenance (`experience add` stores `source=manual`; imported notes should add `--source-ref <url|path|note>` so their origin remains visible). Failed or blocked terminal TASKs are rejected by default; learning one requires `--allow-failure --lesson "<reusable relabel>"`, so the failed trajectory is explicitly relabeled before replay. Add `--failure-cause <planning|context|retrieval|tooling|implementation|verification|integration|runtime|policy|other>` when the relabel has a clear cause classification. Imported/browser/model-derived notes can be stored with `"$FO" experience add <workspace> "<title>" --trust untrusted --source-ref <origin>`; they are visible to manual recall but are excluded from automatic injection unless `--experience-trust all` is set. Promote an imported record only through `"$FO" experience promote <workspace> <slug> --source-ref <origin> --confirm-source-ref <independent-review>`: it requires the stored write-time source to match and records `confirmedBy` before rewriting trust to `trusted` (2606.24322 origin-bound authority, 2606.04329 memory poisoning, 2606.26627 data-centric agent privacy). Use `--supersedes <slug>` on `experience add` or `experience learn` when a newer record deterministically replaces stale/conflicting older memory, and `experience recall --include-superseded --explain` when auditing hidden records. Use `"$FO" experience recall <workspace> --failure-cause <cause> --source task --source-ref <ref> --trust trusted --query "<task>" --min-score <n> --max-age-days <n> --explain` to filter by cause/source/source-ref/trust/supersession/freshness before query ranking, drop weak matches from manual recall, and print score/matched-term/source/source-ref/trust/freshness evidence. Use `"$FO" experience recall <workspace> --query "<task>" --json` when the recalled set itself must be audited: it emits stable method/source/trust/body plus score/matched-term/failure-cause evidence for precision-aware retrieval checks independent of downstream LLM answer quality. Use `"$FO" experience policy <workspace> (<slug>|--query "<task>") [--json]` when an agent or reviewer needs the compact policy/checklist view instead of the full memory body; it deterministically emits `[experience:policy]` plus provenance metadata and requirement/output/audit checklist items (Traversal-as-Policy 2603.05517). Use `"$FO" experience eval <workspace> --cases <json|jsonl> --json` to run local recall cases with expected slugs and receive precision/recall/F1/hit/MRR plus aggregate pass counts (Structured Belief State 2605.11325, RecoAtlas 2605.18805, MemoryAgentBench ICLR 2026, StructMemEval 2602.11243, Agent-Native Memory 2606.24775). Use `"$FO" experience audit <workspace> --json [--max-age-days <n>]` before replay as a store-governance gate: it flags untrusted notes without source refs, trusted imported/manual source-ref notes without `confirmedBy`, untrusted replacements, missing supersession targets, confirmation/source conflicts, and stale trusted records under the active freshness policy (Memory Lifecycle Framework / VMG 2604.16548, Governed Memory 2603.17787, MEMFLOW 2603.15125, evidence tracing 2606.04990). Add `--metadata-only` with recall `--json` when that recall audit crosses a trust boundary: it replaces raw body with `bodySha256` and `bodyChars`, following MRMMIA 2605.27825's memory-membership risk while keeping provenance evidence reproducible. The source/source-ref/trust/confirmation/supersession/freshness filters, `--experience-budget-chars` whole-record packing (MemRefine 2606.13177, Decision-Aware Memory Cards 2606.08151), policy-card rendering, and injected JSON `[experience:meta]` annotations are operator routing/audit controls, not full formal authority boundaries. `workspace context --experience-source --experience-source-ref --experience-limit --experience-budget-chars --experience-trust --experience-max-age-days` and `dispatch --workspace --experience-source --experience-source-ref --experience-limit --experience-budget-chars --experience-trust --experience-max-age-days` reuse the same route, trusted-only default, candidate cap, rendered-character budget, superseded-record hiding, freshness gate, and provenance-bearing prompt rendering for automatic prompt assembly.
...
> Tooling: `FO=orchestration/fuguectl/fuguectl` — unified driver: `doctor` `init` `version` `preflight` `smoke` `fleet` `task new|log|done|handoff|digest` `guard prompt` `review packet` `incident packet` `template` `dispatch` `cache` `integrate` `allocate` `skills` `workspace` `agents` `experience` `plan` `goal` `loop` `run` `summary` `self-harness` `runtime` `selftest`.
...
"$FO" review packet /tmp/fugunano-review-verdict.txt --json > /tmp/fugunano-review-packet.json
