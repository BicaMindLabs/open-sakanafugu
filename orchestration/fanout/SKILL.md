...
> Tooling: `FO=orchestration/fanout/fanout` — unified driver: `doctor` `preflight` `task` `template` `dispatch` `cache` `integrate` `allocate` `skills` `workspace` `plan` `goal` `loop` `run` `summary` `ccb-sync` `selftest`.
